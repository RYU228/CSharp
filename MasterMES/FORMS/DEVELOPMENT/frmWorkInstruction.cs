...
{
    public partial class frmWorkInstruction : frmBase, IButtonAction
    {
        #region 생성자
        public frmWorkInstruction()
        {
            ...
            // 파일 업로드 버튼 생성
            lcgWorkInstructionFile.SetHeaderButtons("FileUpload");
        }
        ...
        public void InitButtonClick()
        {
            try
            {
                ...
                var initSet = SqlHelper.GetDataSet("xp_WorkInstruction", inParams, out string retCode, out string retMsg);

                if (retCode != "00")
                    throw new Exception(retMsg);

                grcProject.SetColumns(initSet.Tables[0]);
                grcWorkInstructionFile.SetColumns(initSet.Tables[1], "GroupSeq", false, false);
                sleCustomID.SetData(initSet.Tables[2], "UseClss");

                // 작업지도서 다운로드
                var riWorkInstructionFileDownload = grvWorkInstructionFile.SetRepositoryItemSimpleButton("FileDownload", RepositoryButtonType.Download);
                riWorkInstructionFileDownload.Click += RiWorkInstructionFileDownload_Click;

                // 작업지도서 삭제
                var riWorkInstructionFileDelete = grvWorkInstructionFile.SetRepositoryItemSimpleButton("FileDelete", RepositoryButtonType.Delete);
                riWorkInstructionFileDelete.Click += RiWorkInstructionFileDelete_Click;

                // 그리드 초기 형식 저장
                DxControl.SaveGridFormat(grvProject, GridFormat.Default);
                DxControl.SaveGridFormat(grvWorkInstructionFile, GridFormat.Default);
            }
            catch (Exception ex)
            {
        ...
        public void NewButtonClick()
        {
            ...
            grvWorkInstructionFile.DeleteAllRows();
        }
        ...
        public void SaveButtonClick()
        {
            try
            {
                ...
                if (SqlHelper.Exist("xp_WorkInstruction", inParams, out _, out _))
                {
                ...
                SqlHelper.Execute("xp_WorkInstruction", inParams, out string retCode, out string retMsg);
        ...
        private void grvProject_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            if (e.FocusedRowHandle >= 0)
            {
                ...
                this.FillGridWorkInstructionFile();
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void lcgWorkInstructionFile_CustomButtonClick(object sender, DevExpress.XtraBars.Docking2010.BaseButtonEventArgs e)
        {
            ...
            try
            {
                ...
                (new FileUpDown()).FillGridAttachFile(this, grcWorkInstructionFile, groupName);
            }
            catch (Exception ex)
            {
                MsgBox.ErrorMessage(ex.CString());
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void RiWorkInstructionFileDownload_Click(object sender, EventArgs e)
        {
            var fileUpDown = new FileUpDown();
            fileUpDown.DownloadAttachFile(this,
                                          txtProjectNo.EditValue.CString().Replace("-", ""),
                                          grvWorkInstructionFile.GetFocusedRowCellValue("GroupSeq").CInt(),
                                          grvWorkInstructionFile.GetFocusedRowCellValue("FileName").CString());
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void RiWorkInstructionFileDelete_Click(object sender, EventArgs e)
        {
            var fileUpDown = new FileUpDown();
            fileUpDown.DeleteAttachFile(this,
                                        txtProjectNo.EditValue.CString().Replace("-", ""),
                                        grvWorkInstructionFile.GetFocusedRowCellValue("GroupSeq").CInt());
            fileUpDown.FillGridAttachFile(this, grcWorkInstructionFile, txtProjectNo.EditValue.CString().Replace("-", ""));
        }
        ...
        private void FillGridProject()
        {
            try
            {
                ...
                var table = SqlHelper.GetDataTable("xp_WorkInstruction", inParams, out _, out _);
        ...
        private void ShowDataProject(string projectNo)
        {
            try
            {
                ...
                var row = SqlHelper.GetDataRow("xp_WorkInstruction", inParams, out _, out _);
        ...
        /// 작업지도서 첨부파일 조회
        /// </summary>
        private void FillGridWorkInstructionFile()
        {
            (new FileUpDown()).FillGridAttachFile(this,
                                                  grcWorkInstructionFile,
                                                  txtProjectNo.EditValue.CString().Replace("-", ""));
        }
        #endregion
